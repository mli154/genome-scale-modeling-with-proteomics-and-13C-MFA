...
% Load the model
model = readCbModel('C:\Users\mli154\Documents\MATLAB\genome scale metabolic modeling/Recon3D_reduced_normalized_abundances.mat');  % Use your SBML (or .mat if you have one)

% Load proteomics data (metabolic proteins CSV)
protT = readtable('C:\Users\mli154\Documents\MATLAB\genome scale metabolic modeling/metabolic_only_proteomics_normalized_abundances.csv');
entrezID = string(protT.ENTREZID);
prot_par = protT.log2_PC3_Parental;
prot_cPACC10 = protT.log2_PC3_cPACC10;

% Cross-group normalization of log2 data
all_log2 = [prot_par; prot_cPACC10];
global_min = min(all_log2, [], 'omitnan');
global_max = max(all_log2, [], 'omitnan');
norm_parental = (prot_par - global_min) / (global_max - global_min);
norm_cPACC10 = (prot_cPACC10 - global_min) / (global_max - global_min);
